...
class random_sensor:public rclcpp::Node{
    public:
        random_sensor():Node("ultrasonic_sensor_array"),mt(rd()){
            min=1.0;
            max=4.0;
            for(int i=0;i<10;i++){
                char topic[]="/ultrasonic_";
                char add[2]={char(48+i),'\0'};
                strcat(topic,add);
                pub_[i]=this->create_publisher<sensor_msgs::msg::Range>(topic,10);
            }
    ...
    private:
        ...
        float min ,max;
        rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr pub_[10];
